(() => {
    ...
    class Popup {
        constructor(options) {
            const headerButton = document.querySelector(".actions-header__reg");
            const headerButtonL = document.querySelector(".actions-header__log");
            ...
            function moveLine(targetLabel) {
                ...
                const containerRect = document.querySelector(".body-popup__items").getBoundingClientRect();
                const lineLeft = labelRect.left - containerRect.left + labelRect.width / 2 - lineHeaderPopup.offsetWidth / 2;
    ...
    window.onload = function() {
        ...
        function documentActions(e) {
            ...
            if (window.innerWidth > 768 && isMobile.any()) {
                if (targetElement.classList.contains("menu__title")) targetElement.closest(".menu__item").classList.toggle("_hover");
            ...
            if (targetElement.classList.contains("search-header__icon-mobile")) targetElement.closest(".search-header").classList.toggle("_active"); else if (!targetElement.closest(".search-header") && document.querySelectorAll(".search-header._active").length > 0) removeClasses(document.querySelectorAll(".search-header._active"), "_active");
        }
        const inputFocus = document.querySelector(".search-header__input");
        const iconSearch = document.querySelector(".search-header__icon-search");
        inputFocus.addEventListener("focus", (() => {
            iconSearch.classList.add("_focused");
        }));
        inputFocus.addEventListener("blur", (() => {
            iconSearch.classList.remove("_focused");
        }));
        const input = document.querySelector(".search-header__input.input");
        const clearButton = document.querySelector(".search-header__delete");
        function showClearButton() {
            if (input.value.length > 0) clearButton.style.display = "inline-block"; else clearButton.style.display = "none";
        }
        function clearInput() {
            input.value = "";
            showClearButton();
        ...
        input.addEventListener("input", showClearButton);
        showClearButton();
        const wrapperLight = document.querySelector(".actions-header__themes");
        const whiteLight = document.querySelector(".actions-header__themes-white");
        function setTheme(theme) {
            document.body.classList.toggle("white-theme", theme === "dark");
            whiteLight.classList.toggle("_white", theme === "dark");
        ...
        wrapperLight.addEventListener("click", toggleTheme);
        const viewPassButton = document.querySelector(".body-popup__viewpass");
